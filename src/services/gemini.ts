...
export async function fetchInterviewQuestions(company: string, jobInfo: string, apiKey?: string): Promise<{ source: string; date: string; industry: string; categories: QuestionCategory[] }> {
  ...
  const prompt = `
    ...
    또한 이 기업이 속한 '산업군(industry)' 정보도 함께 포함해줘.
  ...
  try {
      ...
      config: {
        ...
        responseSchema: {
          ...
          properties: {
            ...
            industry: { type: Type.STRING, description: "기업이 속한 산업군 (예: IT, 금융, 제조 등)" },
            categories: {
              type: Type.ARRAY,
              items: {
                type: Type.OBJECT,
          ...
          required: ["source", "date", "industry", "categories"]
        }
      },
    });

    let text = response.text;
    if (!text) throw new Error("AI로부터 응답을 받지 못했습니다. (Empty response)");

    // Clean potential markdown markers if they exist
    if (text.includes("```")) {
    ...
    if (!data.categories || !Array.isArray(data.categories)) {
      throw new Error("결과 데이터 형식이 올바르지 않습니다. (Missing categories)");
    }
    return data;
  } catch (error) {
  ...
  try {
    const response = await ai.models.generateContent({
      ...
      config: {
        systemInstruction: SYSTEM_INSTRUCTION,
        responseMimeType: "application/json",
        // Removed thinkingLevel temporarily to test stability with complex schema
        responseSchema: {
          type: Type.OBJECT,
          properties: {
            analysis: {
            ...
            versions: {
              ...
              items: {
                ...
                properties: {
                  ...
                  strengths: { type: Type.ARRAY, items: { type: Type.STRING }, description: "이 버전의 주요 강점 2-3가지" },
                  differences: { type: Type.STRING, description: "다른 버전과 차별화되는 핵심 포인트" },
                  star: {
                    type: Type.OBJECT,
                    properties: {
                      s: { type: Type.STRING },
                ...
                required: ["title", "description", "strengths", "differences", "star", "fullText"]
              }
            },
            tips: {
              type: Type.OBJECT,
    ...
    if (!data.analysis || !data.versions || !data.tips) {
      throw new Error("결과 데이터 형식이 올바르지 않습니다. (Missing required fields)");
    }
    return data;
  } catch (error) {
...
export async function startSimulation(
  ...
  const prompt = `
    ...
    지원자의 데이터를 바탕으로 실제 면접처럼 첫 번째 질문을 던져주세요. 
    질문은 한 번에 하나만 하며, 면접관처럼 자연스럽게 질문하세요.
  `;

  try {
    const response = await ai.models.generateContent({
      model: "gemini-3-flash-preview",
      contents: [{ parts: [...personalDataParts, { text: prompt }] }],
      config: {
        systemInstruction: SYSTEM_INSTRUCTION,
        thinkingConfig: { thinkingLevel: ThinkingLevel.LOW },
      },
    });
    return response.text || "첫 번째 질문을 생성하지 못했습니다.";
  } catch (error) {
...
export async function processSimulationAnswer(
  question: string,
  answer: string,
  ...
  const prompt = `
    ...
    위 답변에 대해 실시간 피드백을 제공하고, 다음 면접 질문을 던져주세요.
  ...
  try {
          ...
          properties: {
            feedback: {
              ...
              properties: {
                ...
                sampleAnswer: { type: Type.STRING, description: "더 나은 답변 예시" }
              },
              required: ["score", "strengths", "improvements", "sampleAnswer"]
            },
            nextQuestion: { type: Type.STRING, description: "다음 면접 질문" }
          },
    ...
    console.error("processSimulationAnswer error:", error);
    throw error;
  }
}
